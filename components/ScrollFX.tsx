...
export default function ScrollFX() {
  useEffect(() => {
    ...
    // Dynamic, for the same reason as SmoothScroll: this is mounted in the
    // locale layout, so a static import shipped GSAP + ScrollTrigger in the
    // initial bundle of all 26 routes, several of which declare no `data-fx`
    // element at all. See the note at the top of motion/SmoothScroll.tsx.
    let cancelled = false;
      ...
      const ctx = gsap.context(() => {
        ...
        if (canPin) {
          gsap.utils.toArray<HTMLElement>('[data-fx="pin-scale"]').forEach((card) => {
            ...
            // 🔴 `[data-ambient]` FIRST, and the order is load-bearing. The
            // background loops are now `AmbientVideo`, which withholds the
            // `<video>` until the card is nearly in view — so this effect, which
            // arms once on mount, would find `null` and the scale would silently
            // never happen. The wrapper is always in the DOM and holds both the
            // poster and the footage, so scaling it is also the better effect:
            // the still scales too, instead of snapping when the clip arrives.
            // `video` is kept as a fallback for any card not yet migrated.
            const media = card.querySelector<HTMLElement>("[data-ambient], video");
            const tl = gsap.timeline({
              scrollTrigger: {
                trigger: card,
                start: "top top",
